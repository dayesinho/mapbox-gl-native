...
void NodeMap::startRender(NodeMap::RenderOptions options) {
    ...
    auto currentProjectionMode = map->getProjectionMode();
    mbgl::ProjectionMode projectionOptions;
    if (*currentProjectionMode.axonometric != options.axonometric) {
        projectionOptions.withAxonometric(options.axonometric);
    }

    if (*currentProjectionMode.xSkew != options.xSkew) {
        projectionOptions.withXSkew(options.xSkew);
    }

    if (*currentProjectionMode.ySkew != options.ySkew) {
        projectionOptions.withYSkew(options.ySkew);
    }
